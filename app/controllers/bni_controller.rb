class BniController < ApplicationController
  ...
  def index
    initialize_customer_view("bbe9a4e1ba", "ceiled_transaction_amount", "ceiled_shipping_fee")
    ...
    @basket = Bni.where("number = '#{session[:basket]["basket_number"]}' AND service_id = '#{session[:service].id}' AND operation_id = '#{session[:operation].id}'")
  ...
  def validate_transaction
    ...
    @basket = Bni.where("number = '#{session[:basket]["basket_number"]}' AND service_id = '#{session[:service].id}' AND operation_id = '#{session[:operation].id}'").first rescue nil

    if @error_messages.blank?
      request = Typhoeus::Request.new(@@parameter.bni_billing_request, method: :post, body: {userName: @@parameter.bni_billing_username, password: @@parameter.bni_billing_password, currency: 'XOF', referenceInvoice: @basket.transaction_id, amount: @basket.paid_transaction_amount, serviceFees: @basket.fees, operatorId: @@parameter.bni_operator_id, guceTransactionId: @basket.transaction_id, channelId: @@parameter.bni_channel_id, firstname: @firstname, lastname: @lastname, email: @email, phone: @msisdn}, followlocation: true)
  ...
  def cashout
    ...
    @basket = Bni.find_by_transaction_id(@transaction_id)
  ...
  def transaction_acknowledgement
    OmLog.create(log_rl: 'BNI -- ' + params.to_s) rescue nil
    render text: params
  end
end
