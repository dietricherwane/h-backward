class QashBasketsController < ApplicationController
  ...
  before_action :session_exists?, :except => [:ipn, :transaction_acknowledgement, :payment_result_listener, :generic_ipn_notification]
  # Si l'utilisateur ne s'est pas connecté en passant par main#guard, on le rejette
  before_action :except => [:ipn, :transaction_acknowledgement, :payment_result_listener, :generic_ipn_notification] do |s| s.session_authenticated? end
        ...
        if @basket
          ...
          if (@basket.paid_transaction_amount + @basket.fees) == @transaction_amount.to_f  && (Currency.find_by_code(@basket.paid_currency_id).code.upcase rescue "") == @devise.upcase
            ...
            if (@basket.operation.authentication_token rescue nil) == '3d20d7af-2ecb-4681-8e4f-a585d7700ee4'
              mobile_money_token = 'd0e39ff3'
              reload_request = "#{Parameter.first.gateway_wallet_url}/api/86d138798bc43ed59e5207c664/mobile_money/cashin/QS/#{mobile_money_token}/#{@basket.paymoney_account_number}/#{@basket.original_transaction_amount}/0"
