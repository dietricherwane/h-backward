  ...
  def payment_result_listener
    ...
    if( params[:st] == "Completed" || params[:st] == "Processed" || (params[:st] == "Pending" && ["address", "authorization", "multi-currency"].include?(params[:pending_reason])) )
      ...
      if !@basket.blank?
        ...
        if (@basket.paid_transaction_amount + @basket.fees) == params[:amt].to_f #(@basket.paid_transaction_amount + @basket.fees) == params[:amt].to_f  and (Currency.find_by_id(@basket.paid_currency_id).code.upcase rescue "") == params[:cc].upcase
          @basket.update_attributes(:payment_status => true)
