    ...
    if @response.body == "VERIFIED"
      ...
      if !@basket.blank?
        ...
        if @basket.notified_to_ecommerce != true
          ...
          @request = Typhoeus::Request.new("#{@service.url_to_ipn}?transaction_id=#{@basket.transaction_id}&order_id=#{@basket.number}&status_id=1", followlocation: true, method: :post)
          @request.run
          @response = @request.response
