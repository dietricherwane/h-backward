class MainController < ApplicationController
  ...
  before_action :only => :guard do |s| s.get_service_by_token(params[:currency], params[:service_token], params[:operation_token], params[:order], params[:transaction_amount], params[:id], params[:paymoney_account_number], params[:paymoney_password]) end
  ...
  before_action :filter_connections, only: [:guard, :index]
  ...
  before_action :session_exists?, :except => [:ipn, :transaction_acknowledgement, :mtn_deposit_from_ussd, :mtn_payment_from_ussd]
  ...
  def select_layout
    session[:service].authentication_token == '57813dc7992fbdc721ca5f6b0d02d559' ? 'guce' : 'main'
  end
  ...
  def index
    ...
    @available_wallets = session[:service].available_wallets.where(
      published: true, 
      wallet_id: session[:service].wallets.where(country_id: @international.id).map{|w| w.id}
    )
  end
end
