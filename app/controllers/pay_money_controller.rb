  ...
  def process_payment
    ...
    if @error
      ...
      if @status.to_s.strip == "1"
        ...
        redirect_to "#{session[:service].url_on_success}?transaction_id=#{@basket.transaction_id}&order_id=#{@basket.number}&status_id=1&wallet=05ccd7ba3d&transaction_amount=#{session[:basket]['transaction_amount']}"
